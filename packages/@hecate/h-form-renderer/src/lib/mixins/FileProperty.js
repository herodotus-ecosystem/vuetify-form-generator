...
export default {
    computed: {
        isFileProp() {
            if (this.fullSchema.type === 'string' && (this.fullSchema.contentMediaType || this.display === 'file'))
                return true;
            if (
                this.fullSchema.type === 'array' &&
                this.fullSchema.items &&
                this.fullSchema.items.type === 'string' &&
                (this.fullSchema.items.contentMediaType ||
                    this.display === 'file' ||
                    this.fullSchema.items['x-display'] === 'file')
            )
                return true;
            if (this.fullSchema.type === 'object' && (this.fullSchema.contentMediaType || this.display === 'file'))
                return true;
            if (
                this.fullSchema.type === 'array' &&
                this.fullSchema.items &&
                this.fullSchema.items.type === 'object' &&
                (this.fullSchema.items.contentMediaType ||
                    this.display === 'file' ||
                    this.fullSchema.items['x-display'] === 'file')
            )
    ...
    methods: {
        renderFileProp(h) {
            ...
            if (this.fullSchema.items && this.fullSchema.items.contentMediaType)
                attrs.accept = this.fullSchema.items.contentMediaType;
                ...
                change: async (files) => {
                    if (this.fullSchema.type === 'array') {
                        const contents = await Promise.all(
                            files.map((file) =>
                                getFileResult(file, this.fullSchema.items, this.fullOptions.filesAsDataUrl)
                            )
                        );
                        this.input(contents);
                        this.change(contents);
                    } else {
                        const content = await getFileResult(
                            files,
                            this.resolvedSchema,
                            this.fullOptions.filesAsDataUrl
                        );
                        this.input(content);
                        this.change(content);
                    }
