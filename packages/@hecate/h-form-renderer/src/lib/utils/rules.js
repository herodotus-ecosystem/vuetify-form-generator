export const getRules = (fullSchema, options, required, isOneOfSelect) => {
    ...
    if (fullSchema.type === 'string' && fullSchema.minLength !== undefined) {
        const msg = options.messages.minLength.replace(
            '{minLength}',
            fullSchema.minLength.toLocaleString(options.locale)
        );
    ...
    if (fullSchema.type === 'string' && fullSchema.maxLength !== undefined) {
        const msg = options.messages.maxLength.replace(
            '{maxLength}',
            fullSchema.maxLength.toLocaleString(options.locale)
        );
    ...
    if (fullSchema.enum) {
        rules.push(
            (val) =>
                val === undefined ||
                val === null ||
                !!fullSchema.enum.find((item) => JSON.stringify(item) === JSON.stringify(val)) ||
                ''
        );
    }
    if (fullSchema.type === 'array' && fullSchema.items.enum) {
        rules.push(
            (val) =>
                ...
                val === null ||
                !val.find(
                    (valItem) => !fullSchema.items.enum.find((item) => JSON.stringify(item) === JSON.stringify(valItem))
                ) ||
                ''
        );
    }
    if (isOneOfSelect && fullSchema.type !== 'array') {
        rules.push(
            (val) => val === undefined || val === null || !!fullSchema.oneOf.find((item) => item.const === val) || ''
        );
    }
    if (isOneOfSelect && fullSchema.type === 'array') {
        rules.push(
            (val) =>
                val === undefined ||
                val === null ||
                !val.find((valItem) => !fullSchema.items.oneOf.find((item) => item.const === valItem)) ||
                ''
        );
    }
