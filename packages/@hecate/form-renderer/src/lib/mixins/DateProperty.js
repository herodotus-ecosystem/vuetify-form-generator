...
const getDateTimeWithOffset = (date) => {
    ...
    const offsetAbs = `${padTimeComponent(parseInt(Math.abs(offsetMinutes / 60)))}:${padTimeComponent(
    ...
    return `${date.getFullYear()}-${padTimeComponent(date.getMonth() + 1)}-${padTimeComponent(
        date.getDate()
    )}T${padTimeComponent(date.getHours())}:${padTimeComponent(date.getMinutes())}:${padTimeComponent(
        date.getSeconds()
    )}${offset}`;
};
...
export default {
    ...
    methods: {
        renderDateProp(h) {
            if (this.fullSchema.type !== 'string' || !['date', 'date-time', 'time'].includes(this.fullSchema.format))
                return;
            ...
            if (this.fullSchema.format === 'time') {
                ...
                if (this.value !== this.dateProp.lastValue)
                    this.dateProp.parts = getDateTimeParts(new Date(this.value));
