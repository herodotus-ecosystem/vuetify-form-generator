    ...
    methods: {
        initSelectProp(model) {
            ...
            if ((this.fullSchema.type === 'array' && this.fullSchema.items.enum) || this.fullSchema.enum) {
                this.rawSelectItems =
                    this.fullSchema.type === 'array' ? this.fullSchema.items.enum : this.fullSchema.enum;
            }
            ...
            if (this.fullSchema['x-fromData']) {
                this.$watch(
                    watchPrefix + this.fullSchema['x-fromData'],
                    (val) => {
          ...
          ...this.selectItems.map(item => this.renderCheckboxItem(h, item, on))
            ...
            if (this.fromUrlWithQuery || (this.rawSelectItems && this.rawSelectItems.length > 20)) {
                ...
                if (this.fromUrlWithQuery) {
                    ...
                    on['update:search-input'] = (searchUpdate) => {
                        this.q = searchUpdate;
                    };
                } else {
                    props.filter = (item, q) =>
                        (item[this.itemTitle] || item[this.itemKey]).toLowerCase().includes(q.toLowerCase());
                }
            }
